...
현재 QA 우선순위와 버그 우선순위는 `qa/00_current.md`에 둔다.
...
1. `qa/00_current.md`의 현재 QA 목록을 먼저 확인한다.
...
8. 큰 변경이면 `qa/20_full-check.md`를 확인하고, 필요한 경우 `QAReports` 기능별 종합보고서를 조회한다.
...
현재 QA 상태와 버그 우선순위는 Roadmap에 두지 않고 `qa/00_current.md`에서 관리한다.

## 3.1 QA Documents

| 문서 | 역할 |
| --- | --- |
| `qa/00_current.md` | 현재 수정 중이거나 다시 확인해야 하는 QA만 관리 |
...
- 테스트로 다루기 어려운 내용은 `qa/00_current.md`에 Open 또는 Retest로 남긴다.
...
| `qa/00_current.md` | 현재 QA/버그 우선순위 |
| `qa/20_full-check.md` | 전체 회귀 확인 기준 |
| `qa/90_done/YYYY-MM.md` | 완료된 QA 요약 보관 |
