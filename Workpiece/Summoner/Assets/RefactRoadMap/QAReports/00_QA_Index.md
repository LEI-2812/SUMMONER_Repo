...
> `qa/00_current.md`에는 현재 수정 중이거나 다시 확인해야 하는 항목만 남긴다.
> 전체 회귀 확인은 `qa/20_full-check.md`, 완료된 QA는 `qa/90_done/YYYY-MM.md`에 둔다.
> 이 `QAReports` 폴더의 md/csv는 기능별 QA 종합보고서와 이전 상세 근거 보관용으로 유지한다.

현재 작업 큐는 `qa/00_current.md`다.
전체 회귀 확인은 `qa/20_full-check.md`, 완료된 QA는 `qa/90_done/YYYY-MM.md`에 둔다.
...
- 새 QA 항목은 `qa/00_current.md`에만 작성한다.
- 완료된 QA는 `qa/90_done/YYYY-MM.md`로 옮긴다.
- 큰 변경 후 확인 목록은 `qa/20_full-check.md`를 사용한다.
...
현재 우선순위는 `qa/00_current.md`에서 관리한다.
