...
// 역할: 현재 플레이어 소환수들의 다음 공격 가능성을 예측 목록으로 만든다.
public class PlayerAttackPrediction : MonoBehaviour
{
    // 역할: 현재 플레이트 위치와 가장 가까운 적 위치를 조회할 때 사용한다.
    private PlateController plateController;

    // 역할: 소환수 타입별 예측 규칙을 실행할 예측기 목록을 저장한다.
    private List<IAttackPrediction> attackPredictions;

    // 역할: 같은 오브젝트에 붙은 플레이트 컨트롤러와 소환수별 예측기를 미리 찾아둔다.
    private void Awake()
    {
    ...
    public List<AttackPrediction> GetPlayerAttackPredictionList(List<Plate> playerPlates, List<Plate> enermyPlates)
    {
        ...
        foreach (Plate plate in playerPlates)
        {
            ...
            if (summon != null)
            {
                ...
                int attackIndex = plateController.GetClosestEnermyPlateIndexExcept(summon);
                if (attackIndex == -1)
                {
    ...
    // 역할: 디버그 로그에서 예측 결과를 사람이 바로 읽을 수 있는 문장으로 바꾼다.
